...
def test_verify_google_token_disabled_when_no_client_id(monkeypatch):
    """verify_google_token raises 501 when GOOGLE_CLIENT_ID is empty."""
    import auth
    monkeypatch.setattr(auth, "GOOGLE_CLIENT_ID", "")
    with pytest.raises(HTTPException) as exc_info:
        asyncio.run(auth.verify_google_token("some-token"))
...
def test_verify_google_token_invalid(monkeypatch):
    ...
    fake_verify = mock.MagicMock(side_effect=ValueError("Token is not valid"))
    monkeypatch.setattr("google.oauth2.id_token.verify_oauth2_token", fake_verify)

    with pytest.raises(HTTPException) as exc_info:
        asyncio.run(auth.verify_google_token("bad-token"))
    assert exc_info.value.status_code == 401
...
def test_verify_google_token_valid(monkeypatch):
    ...
    fake_verify = mock.MagicMock(return_value=expected_payload)
    monkeypatch.setattr("google.oauth2.id_token.verify_oauth2_token", fake_verify)

    result = asyncio.run(auth.verify_google_token("good-token"))
